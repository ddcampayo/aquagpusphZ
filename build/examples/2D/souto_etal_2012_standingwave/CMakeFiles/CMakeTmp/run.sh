...
if [[ $1 == "--run" ]]; then
    ...
    rm -f sensors_h.out
    /usr/local/share/aquagpusph/examples/2D/souto_etal_2012_standingwave/cMake/../Create.py
    /usr/local/bin/AQUAgpusph2D -i Main.xml
elif [[ $1 == "--plot" ]]; then
    python /usr/local/share/aquagpusph/examples/2D/souto_etal_2012_standingwave/cMake/../plot_$2.py
else
    echo ""
